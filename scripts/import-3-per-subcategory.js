...
async function scrapeAmazonSearch(query) {
  try {
    ...
    items.each((i, el) => {
      if (products.length >= 10) return false; // Grab first 10 to choose from
      ...
      if (!title || !relativeUrl || !price || !img || !asin) {
         console.log(`[Debug] Skipped item due to missing fields: title=${!!title}, url=${!!relativeUrl}, price=${!!price} (str: ${priceStr}), img=${!!img}, asin=${!!asin}`);
      }
...
async function run() {
  console.log('--- STEP 1: Subcategory Diagnostics ---');
  ...
  if (targetList.length === 0) {
    console.log('All subcategories have at least 3 products. Exiting.');
    return;
  }
  ...
  for (const target of targetList) {
    ...
    // Delay before moving to the next subcategory (unless we are totally done)
    const isLastItemOverall = (target === targetList[targetList.length - 1]);
