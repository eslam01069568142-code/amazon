...
export function removeAdjectives(text: string): string {
...
export function findCategory(textList: string[]): { parent: string, child: string } | null {
...
export async function scrapeAndInsertProduct(
  url: string, 
  isPreview: boolean, 
  trackingId: string, 
  ...
  if (!cleanUrl) return { success: false, status: 'Failed', error: 'Invalid URL' };
  ...
  try {
    ...
    if (asin && !isPreview) {
      const { data: existingProd } = await supabaseAdmin.from('products').select('id, category').eq('id', productId).single();
      if (existingProd) {
        return { success: false, status: 'Duplicate', error: 'المنتج موجود بالفعل' };
      }
    }
    ...
    if (!response.ok) {
      return { success: false, status: 'Failed', error: `Failed to fetch: ${response.status}` };
    }
    ...
    if (!title || title === '' || title.includes('Amazon.eg')) {
      return { success: false, status: 'Failed', error: 'Failed to extract product data (possible CAPTCHA)' };
    }
    ...
    if (!mainImage) {
      return { success: false, status: 'Failed', error: 'Failed to extract images' };
    }
    ...
    let match = null;
    if (breadcrumbs.length > 0) match = findCategory(breadcrumbs);
    if (!match) match = findCategory([title, brand]);
    
    let assignedCategoryId = targetCategory || '';
    
    if (match && !assignedCategoryId) {
    ...
    if (needsPrice || needsCategory) {
      ...
      return { success: false, status: 'NeedsInput', error: combinedErrors, product, needsPrice, needsCategory };
    }

    if (isPreview) {
      return { success: true, status: 'Preview', product };
    }
    ...
    if (error) {
      return { success: false, status: 'Failed', error: 'Database insert error' };
    }
    ...
    return { success: true, status: 'Success', product, asin };

  } catch (err) {
    console.error(`Error processing ${cleanUrl}:`, err);
    return { success: false, status: 'Failed', error: 'Internal processing error' };
  }
}
